...
async function loadSymbolDataFromFile(
  symbol: string
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      console.log(
        `Arquivo de dados para ${symbol} (${fileName}) não encontrado. Iniciando vazio.`
      );
    } else {
...
async function saveSymbolDataToFile(
  symbol: string,
...
async function getTopGainersFromBinance(
  ...
  try {
    ...
    const tickersArray: BinanceDailyStat[] = Array.isArray(tickersResponse)
      ? tickersResponse
      : tickersResponse
      ? [tickersResponse as any]
      : []; // Assegura que é um array

    if (
      tickersArray.length === 0 &&
      !Array.isArray(tickersResponse) &&
      tickersResponse
    ) {
      console.warn(
        "prevDay retornou um objeto único, não um array. Resposta:",
        tickersResponse
      );
    }
    ...
    console.log(
      `Top ${topN} gainers (24h) selecionados:`,
      topGainers.join(", ")
    );
    return topGainers;
  } catch (error) {
...
const main = async () => {
  ...
  if (config.dynamicSymbols.enabled) {
    ...
    console.log(
      "Monitoramento dinâmico desabilitado. Usando fallback:",
  ...
  for (const symbol of symbolsToMonitor) {
    ...
    for (const interval of config.intervals) {
      const intervalData = loadedSymbolData[interval] || {
        emaHistory: [],
        rsiClosePrices: [],
      };
    ...
    symbolStrategyStates[symbol] = {
      majorRallyActive: false,
      rallyReferencePrice: null,
      rallyPeakPriceSinceActive: null,
      rallyDetectedOnTimeframe: null,
  ...
  if (streams.length === 0) {
    console.error("Nenhum stream para inscrever. Encerrando.");
    return;
  }
  console.log(
    `Iniciando monitoramento para ${streams.length} streams (ex: ${streams
      .slice(0, 3)
      .join(", ")}...).`
  );
  ...
  binance.websockets.subscribeCombined(
    ...
    (klineEventData: KlineEvent) => {
      if (symbolsToMonitor.includes(klineEventData.s)) {
        handleKlineDataWithIndicators(klineEventData).catch((e) =>
          console.error(
            `Erro em handleKlineData [${klineEventData.s}@${klineEventData.k.i}]: ${e}`,
            e.stack
          )
        );
      }
...
const handleKlineDataWithIndicators = async (
  klinePayload: KlineEvent
): Promise<void> => {
  if (!klinePayload || !klinePayload.k) {
    console.warn(
      "Payload da vela inválido:",
      JSON.stringify(klinePayload, null, 2)
    );
    return;
  }
  ...
  if (!tfState || !stratState) {
    console.warn(
      `Estado tfState ou stratState não inicializado para ${eventSymbol}@${interval}.`
    );
    return;
  }
  if (!kline.x) return;
  ...
  console.log(
    `\n--- [${eventSymbol} @ ${interval} | ${eventTime}] Fech: ${closePrice.toFixed(
      2
    )} ---`
  );

  //#region --- CÁLCULO DE EMA ---
  ...
  if (tfState.emaHistory.length > config.historySaveLimit)
    tfState.emaHistory.shift();
  // #endregion

  // #region --- CÁLCULO DE RSI ---
  tfState.allClosePriceHistoryForRSI.push(closePrice);
  if (tfState.allClosePriceHistoryForRSI.length > config.historySaveLimit)
    tfState.allClosePriceHistoryForRSI.shift();

  if (tfState.allClosePriceHistoryForRSI.length <= config.rsiPeriod) {
    console.log(
      `[${eventSymbol}@${interval}] MME(${
        config.emaPeriod
      }): ${currentEMA.toFixed(4)}`
    );
    console.log(
      `[${eventSymbol}@${interval}] RSI: Aguardando ${
        config.rsiPeriod + 1
      }p. (${tfState.allClosePriceHistoryForRSI.length})`
    );
  } else {
    const relevantPrices = tfState.allClosePriceHistoryForRSI.slice(
      -(config.rsiPeriod + 1)
    );
    ...
    if (
      ...
      (relevantPrices.length === config.rsiPeriod + 1 &&
        priceChanges.length === config.rsiPeriod)
    ) {
      avgGain = gains.reduce((s, v) => s + v, 0) / config.rsiPeriod;
      avgLoss = losses.reduce((s, v) => s + v, 0) / config.rsiPeriod;
    } else {
      avgGain =
        (tfState.previousAverageGain * (config.rsiPeriod - 1) +
          gains[gains.length - 1]) /
        config.rsiPeriod;
      avgLoss =
        (tfState.previousAverageLoss * (config.rsiPeriod - 1) +
  ...
  if (config.strategy.rallyDetectionTimeframes.includes(interval)) {
    if (!stratState.majorRallyActive) {
      if (
        tfState.allClosePriceHistoryForRSI.length >=
        config.strategy.rallyLookbackPeriods
      ) {
        const lookbackPrices = tfState.allClosePriceHistoryForRSI.slice(
          -config.strategy.rallyLookbackPeriods
        );
        const lowWatermark = Math.min(...lookbackPrices);
        if (
          closePrice >
          lowWatermark * (1 + config.strategy.rallyMinPercentIncrease / 100)
        ) {
          ...
          console.log(
            `\x1b[36m\x1b[1m[${eventSymbol}] SÍMBOLO ARMADO! Valorização de ${(
              ((closePrice - lowWatermark) / lowWatermark) *
              100
            ).toFixed(
              2
            )}% detectada em ${interval} (fundo ref.: ${lowWatermark.toFixed(
              2
            )}). Aguardando RSI em ${config.strategy.entryTimeframesInSequence.join(
              "/"
            )}.\x1b[0m`
          );
        }
      ...
      // Macro Rally JÁ ESTÁ ATIVO
      if (closePrice > (stratState.rallyPeakPriceSinceActive || 0)) {
        stratState.rallyPeakPriceSinceActive = closePrice;
      }
      if (
        stratState.rallyPeakPriceSinceActive &&
        closePrice <
          stratState.rallyPeakPriceSinceActive *
            (1 - config.strategy.rallyPrimedInvalidationDropPercent / 100)
      ) {
        console.log(
          `\x1b[31m[${eventSymbol}] SÍMBOLO DESARMADO (Macro Rally Encerrado). Preço (${closePrice.toFixed(
            2
          )} @ ${interval}) caiu de ${stratState.rallyPeakPriceSinceActive.toFixed(
            2
          )} (pico desde armado).\x1b[0m`
        );
  ...
  if (config.strategy.entryTimeframesInSequence.includes(interval)) {
    if (
      ...
      const currentIndex =
        config.strategy.entryTimeframesInSequence.indexOf(interval);
      for (let i = 0; i < currentIndex; i++) {
        // Verifica se TFs ANTERIORES na sequência já foram usados
      ...
      if (canTriggerThisTimeframe) {
        console.warn(
          `\x1b[33m\x1b[1m>>> GATILHO DE ENTRADA [${eventSymbol}@${interval}]: RSI (${tfState.rsiValue.toFixed(
            2
          )}) em sobrevenda APÓS VALORIZAÇÃO (detectada em ${
            stratState.rallyDetectedOnTimeframe
          }, pico armado ${stratState.rallyPeakPriceSinceActive?.toFixed(
            2
          )}). COMPRAR!\x1b[0m`
        );
        ...
        if (allEntryTfsUsed) {
          console.log(
            `\x1b[35m[${eventSymbol}] Todas as oportunidades de entrada (${config.strategy.entryTimeframesInSequence.join(
              "/"
            )}) para o rally atual foram utilizadas. Desarmando o símbolo para este rally.\x1b[0m`
          );
          stratState.majorRallyActive = false;
          stratState.rallyReferencePrice = null;
          stratState.rallyPeakPriceSinceActive = null;
